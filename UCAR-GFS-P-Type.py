...
def readCloudData(extent,prateShape,i):
    ...
    ds = xr.open_dataset('http://nomads.ncep.noaa.gov:80/dods/gfs_0p25/gfs20240121/gfs_0p25_06z')
...
def buildMap(i, ax, extent):            
    ...
    prate_rain = np.where(crain[::-1]>0.5,prate,0)
    prate_snow = np.where(csnow[::-1]>0.5,prate,0)
    prate_zr = np.where(czr[::-1]>0.5,prate,0)
    prate_ip = np.where(cip[::-1]>0.5,prate,0)
...
def startScript():
    ...
    for i in np.arange(6,246,6):
        i = iCheck(i)
        print(i)
        url = f'https://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/v16.3/gfs.20240121/06/atmos/gfs.t06z.sfluxgrbf{i}.grib2'
